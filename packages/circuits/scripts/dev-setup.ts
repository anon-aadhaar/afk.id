...
const PHASE1_PATH = path.join(BUILD_DIR, "powersOfTau28_hez_final_20.ptau");
...
const CIRCUITS = [
  "afk.circom",
  // "issuers/anon-aadhaar.afk.circom",
];
...
async function exec() {
  ...
  for (const circuitPath of CIRCUITS) {
    const circuitName = circuitPath.split("/").pop()!.replace(".circom", "");
    
    log(`\nGenerating keys for ${circuitName}`);

    const r1cPath = path.join(BUILD_DIR, `${circuitName}.r1cs`);
    if (!fs.existsSync(r1cPath)) {
      throw new Error(`${r1cPath} does not exist.`);
    }
    ...
    fs.copyFileSync(
      ...
      path.join(ARTIFACTS_DIR, `${circuitName}.r1cs`)
    );
    fs.copyFileSync(
      path.join(BUILD_DIR, `${circuitName}_js/${circuitName}.wasm`),
      path.join(ARTIFACTS_DIR, `${circuitName}.wasm`)
    );
    ...
    await generateKeys(
      ...
      r1cPath,
      zKeyPath,
      path.join(ARTIFACTS_DIR, `${circuitName}.vkey.json`),
      path.join(
        ARTIFACTS_DIR,
