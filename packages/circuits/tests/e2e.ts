...
const SECRET = 209307326894871195295056404570;
...
async function generateProof(circuitName: string, inputs: any) {
  ...
  const wc = require(path.join(
    BUILD_DIR,
    `${circuitName}_js/witness_calculator.js`
  ));
  ...
  const vkey = JSON.parse(
    fs
      .readFileSync(path.join(BUILD_DIR, `artifacts/${circuitName}.vkey.json`))
      .toString()
  );
...
async function e2e() {
  ...
  const aadhaarAFKProof = await generateProof('anon-aadhaar.afk', aadhaarAFKInputs);
  console.log("Done ✔");
  ...
  const afkVerifierProof = await generateProof("afk", {
    ...
    claimKeys: [BigInt(computeClaimKeyFromName("bio.country")).toString(), "0", "0", "0", "0"],
    claimValues: [stringToPackedInts("India")[0].toString(), "0", "0", "0", "0"],
    scope: APP_ID,
    message: 8827718,
  });
  ...
  console.log(afkVerifierProof.publicSignals[1] === aadhaarAFKProof.publicSignals[3]);
}
